...
@export var reset_time_ms := 1.5 * 1000.0
...
func _process(delta: float) -> void:
	...
	match self.state:
		STATE_NORMAL:
			...
			if self.timer + max_time_ms <= now:
				self.timer = now
				self.state = STATE_RESETTING
				timer_label.value = 0.0
			else:
				timer_label.value = (max_time_ms - (now - timer)) / 1000.0
		STATE_RESETTING:
			var reset_amount := (now - timer) / reset_time_ms
			if reset_amount >= 1.0:
				self.timer = now
				...
				timer_label.value = reset_amount * max_time_ms / 1000.0
